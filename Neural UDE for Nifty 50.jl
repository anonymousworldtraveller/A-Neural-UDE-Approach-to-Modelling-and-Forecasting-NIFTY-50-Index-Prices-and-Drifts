...
df = CSV.read("putappropriatelocationaddresshere/Nifty Data.csv", DataFrame)

# *Select training range (50 days) 
Prices_raw = log.(df.Price[2419:2419+49])  
Drifts_raw = df.log_return[2419:2419+49]
...
Prices_all_raw = log.(df.Price[2419:2419+59])  
Drifts_all_raw  = df.log_return[2419:2419+59]
...
parameter_array = Float64[0.0007844783441666809 0.0138] #*paramters will change according to training period
...
Prices_all = log.(df.Price[2419:2419+59])
Drifts_all = df.log_return[2419:2419+59]
...
plot(t_forecast, Prices_all, label="Actual Price", color="red", lw=2, 
     xlabel="Time", ylabel="Normalized Value of NIFTY 50", 
     title="NIFTY 50 Actual vs. Predicted Values", 
     titlefontsize=11)
