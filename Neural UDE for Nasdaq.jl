...
#This code is for the last case of 5 training points and 10 forecasting points. For other cases, only the starred
#commented sections need to be altered by changing the indexation.  

# load and pre-process the data 
df = CSV.read("C:/Users/ISHAAN/Downloads/NASDAQ.csv", DataFrame)

# *Select training range (5 days) 
Prices_raw = log.(df.Price[2501:2501+4])  
Drifts_raw = df.log_return[2501:2501+4]
...
# *Full window of 15 points
Prices_all_raw = log.(df.Price[2501:2501+14])  
Drifts_all_raw  = df.log_return[2501:2501+14]
...
parameter_array = Float64[0.00307351861973755 0.138] #*paramters will change according to training period
...
Prices_all = log.(df.Price[2501:2501+14])
Drifts_all = df.log_return[2501:2501+14]
...
plot(t_forecast, Prices_all, label="Actual Price", color="red", lw=2, 
     xlabel="Time", ylabel="Normalized Value of NASDAQ", 
...
vline!([5.0], label="Forecast Start", color=:black, lw=2, ls=:dot)
